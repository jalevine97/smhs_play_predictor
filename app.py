...
def load_data():
    ...
    df["trend_weight"] = df.groupby("game")["play_type_binary"].transform(lambda s: s.shift(1).expanding().mean().fillna(0.5))
...
def build_feature_matrix(df_subset):
    ...
    return pd.concat([num_df, cat_encoded], axis=1)

# --- UI ---
st.sidebar.title("Play Selector")
games = df["game"].unique()
selected_game = st.sidebar.selectbox("Game", games)
plays = df[df["game"] == selected_game]["play_#"].dropna().astype(int).unique()
selected_play = st.sidebar.selectbox("Play Number", sorted(plays))

play_row = df[(df["game"] == selected_game) & (df["play_#"] == selected_play)]

if play_row.empty:
    st.warning("No data found for that play.")
    st.stop()
...
# Spinner with football GIF
with st.spinner("Making a prediction..."):
    st.markdown(
        "<div style='text-align:center'><img src='https://media.giphy.com/media/5KwNWGLwZv0Tm/giphy.gif' width='150'></div>",
